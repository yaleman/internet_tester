...
""" does internet test things """

failed_module = False #pylint: disable=invalid-name
try:
    import sys
    import subprocess
    import traceback
    ...
    print(f"Failed to import module: {importerror}", file=sys.stderr)
    failed_module = True
...
def ping(endpoint):
    """ use the ping command to check a server """

    command = "/sbin/ping -c 5 {}".format(endpoint)
    response = subprocess.run(command.split(), stdout=subprocess.DEVNULL)
...
def get_default_route():
    ...
    default_route = [
        route
        for route in routes.decode("utf-8").split("\n")
    ...
    if BASICS:
        ...
        if DNSRESOLVE:
            ...
            if nameservers_good != len(get_nameservers()):
                txt.insert(
                    "insert", "One or more nameservers failed to resolve google.com\n"
                )
            elif nameservers_good == 0:
                txt.insert(
                    "insert", "No nameservers resolved google.com successfully\n"
                )
                return False
        ...
        if DOWNLOADTEST:
            ...
            txt.insert(
                "insert",
                "downloading {} 10 MB files. This could take some time.".format(
                    len(testfilenames["ten"])
                ),
            )
            root.update()
            failed = 0
